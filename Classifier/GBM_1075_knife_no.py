...
for train, test in loo.split(X): 
    ...
    hist=gbm.fit(X_train, y_train)
    y_predict_score=gbm.predict_proba(X_test) 
...
yscore_sum.to_csv('yscore_GBM_1075_knife_no.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('ytest_GBM_1075_knife_no.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='GBM ROC (area = %0.2f%%)' % auc_score)
...
data_csv.to_csv('GBM_1075_knife_no.csv')
